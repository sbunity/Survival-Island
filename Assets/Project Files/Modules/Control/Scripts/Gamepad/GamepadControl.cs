    ...
    public class GamepadControl : MonoBehaviour, IControlBehavior
    {
        ...
        public static bool WasButtonReleasedThisFrame(GamepadButtonType button)
        {
            ...
            switch (button)
            {
                ...
                case GamepadButtonType.Start:
                    return Gamepad.current.startButton.wasReleasedThisFrame;
                case GamepadButtonType.Select:
                    return Gamepad.current.selectButton.wasReleasedThisFrame;
                ...
                case GamepadButtonType.L3:
                    return Gamepad.current.leftStickButton.wasReleasedThisFrame;
                case GamepadButtonType.R3:
                    return Gamepad.current.rightStickButton.wasReleasedThisFrame;
        ...
        private void GamepadButtonUpdate(GamepadButtonType type, bool isPressed)
        {
            if (isPressed)
            {
                if (!gamepadButtonPressedTime.ContainsKey(type))
                {
                    gamepadButtonPressedTime.Add(type, Time.time);
                }
                else
                {
                    gamepadButtonPressedTime[type] = Time.time;
                }
            }
            else
            {
                if (gamepadButtonPressedTime.ContainsKey(type))
                    gamepadButtonPressedTime.Remove(type);
            }
        }
