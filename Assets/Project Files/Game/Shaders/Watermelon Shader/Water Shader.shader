    ...
    SubShader
    {
        ...
        #define VIEW_DIR
            ...
            float GetDepth(float4 screenPos)
            {
                ...
                {
                    float surfaceRawDepth = screenPos.z / screenPos.w;

                    #if UNITY_REVERSED_Z
                        rawDepth = 1.0 - rawDepth;
                        surfaceRawDepth = 1.0 - surfaceRawDepth;
                    #endif

                    float sceneDepth = lerp(_ProjectionParams.y, _ProjectionParams.z, rawDepth);

                    float surfaceDepth = lerp(_ProjectionParams.y, _ProjectionParams.z, surfaceRawDepth);
