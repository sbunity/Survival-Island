...
namespace Watermelon
{
    public class PlayerBehavior : MonoBehaviour, ICharacter, ICombatTarget, ICharacterGraphics<PlayerGraphics>, IResourceGiver, IResourceTaker, IHitter
    {
        ...
        #region Drop
        public void ShowFullFloatingText(bool ignoreDelay = false, string customize = "")
        {
            if (ignoreDelay || Time.time > lastFullMessageTime)
            {
                FloatingTextController.SpawnFloatingText(FULL_FLOATING_TEXT_HASH, customize != "" ? customize : "FULL!", transform.position + new Vector3(0, 2, 0), Quaternion.identity, 1.0f, Color.red);
