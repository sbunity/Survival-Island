...
namespace Watermelon
{
    public class ResourceSourceBehavior : AbstractHitableBehavior, IGroundOpenable, IWorldElement
    {
        ...
        [BoxFoldout("Optimization", "Optimization")]
        [SerializeField] GameObject optimizedHarvestStagesPrefab;
        [BoxFoldout("Optimization")]
        [SerializeField] GameObject defaultVisualsObject;
        ...
        private ResourceOptimizedHarvestStages optimizedHarvestStages;
        ...
        private void Awake()
        {
            ...
            if(optimizedHarvestStagesPrefab == null)
            {
        ...
        {
            if(!harvestStages.IsNullOrEmpty())
            {
        ...
        {
            if (optimizedHarvestStages != null)
            {
                optimizedHarvestStages.Unload();

                Destroy(optimizedHarvestStages.gameObject);

                defaultVisualsObject.SetActive(true);

                optimizedHarvestStages = null;
            }
            else
            {
                for (int i = 0; i < harvestStages.Count; i++)
        ...
        public override void GetHit(Vector3 hitSourcePosition, bool drop = true, IHitter resourcePicked = null)
        {
            ...
            if (optimizedHarvestStagesPrefab != null && optimizedHarvestStages == null)
            {
                optimizedHarvestStages = Instantiate(optimizedHarvestStagesPrefab, defaultVisualsObject.transform.parent).GetComponent<ResourceOptimizedHarvestStages>();
                optimizedHarvestStages.Init();

                Transform optimizedTransform = optimizedHarvestStages.transform;
                Transform defaultVisualsTransform = defaultVisualsObject.transform;

                optimizedTransform.localScale = defaultVisualsTransform.localScale;
                optimizedTransform.SetLocalPositionAndRotation(defaultVisualsTransform.localPosition, defaultVisualsTransform.localRotation);

                harvestStages = optimizedHarvestStages.HarvestStages;

                defaultVisualsObject.SetActive(false);
