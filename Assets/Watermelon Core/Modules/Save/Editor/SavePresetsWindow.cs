...
namespace Watermelon
{
    public class SavePresetsWindow : EditorWindow
    {
        ...
        protected void OnEnable()
        {
            ...
            {
                directory = SavePresets.GetName(directoryEntries[i]);
                sortType = GetSortType(directory);
                saveEntries = Directory.GetDirectories(directoryEntries[i]);
                ...
                for (int j = 0; j < saveEntries.Length; j++)
                {
                    if (!saveEntries[j].EndsWith(SavePresets.META_SUFFIX))
                    {
                        creationTime = Directory.GetCreationTimeUtc(saveEntries[j]);
                        allSavePresets.Add(new SavePreset(SavePresets.GetName(saveEntries[j]), creationTime, saveEntries[j], directory));
                    }
                }
            }
        ...
        {
            string originalFolderPath = EditorUtility.OpenFolderPanel("Select folder to import", string.Empty, string.Empty);

            if(originalFolderPath.Length == 0)
            {
                return;
            }

            //string name = originalFolderPath.Substring(originalFolderPath.LastIndexOf('/') + 1);
            string name = SavePresets.GetName(originalFolderPath);
            ...
            string newDirectoryPath = Path.Combine(defaultDirectoryPath, name);

            if (!Directory.Exists(newDirectoryPath)) //Creating SavePresets folder
            {
                Directory.CreateDirectory(newDirectoryPath);
            }

            string[] saveFiles = Directory.GetFiles(originalFolderPath);
            string[] presetFilePaths = new string[saveFiles.Length];
            SavePresets.DeleteFiles(newDirectoryPath); // avoiding situation when new save partially overrides old save that has more files

            for (int i = 0; i < presetFilePaths.Length; i++)
            {
                presetFilePaths[i] = Path.Combine(newDirectoryPath, SavePresets.GetName(saveFiles[i]));
